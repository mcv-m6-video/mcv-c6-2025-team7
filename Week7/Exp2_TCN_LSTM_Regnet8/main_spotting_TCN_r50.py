...
from model.model_spotting_TCN_r50 import Model
import wandb
import optuna
...
def run_training(args, trial):
    ...
    if trial:
        execution_name = f'TPNr50_TCN_finetuning{str(trial.number)}'
    
    else:
        execution_name = f'TPNr50_TCN'

    wandb.init(
        project='Week7_TPN-R50_TCN',
        entity='mcv-c6g7',
        name=execution_name,
    ...
    if not args.only_test:
        ...
        for epoch in range(epoch, num_epochs):
            ...
            if args.save_dir is not None:
                ...
                if better:
                    torch.save( model.state_dict(), os.path.join(ckpt_dir, f'checkpoint_TCN_r8{args.batch_size}_{args.learning_rate}_{args.num_epochs}_{args.downsample_factor}_{args.warm_up_epochs}.pt') )
    
    # model_parameters = model.get_model_parameters()
    # wandb.log({"model_params": model_parameters})

    print('START INFERENCE')
    model.load(torch.load(os.path.join(ckpt_dir, f'checkpoint_TCN_r8{args.batch_size}_{args.learning_rate}_{args.num_epochs}_{args.downsample_factor}_{args.warm_up_epochs}.pt')))
    ...
    with open(f"results/TCN_r8{args.batch_size}_{args.learning_rate}_{args.num_epochs}_{args.downsample_factor}_{args.warm_up_epochs}.txt", "w") as f:
        f.write(result_text)
    ...
    with open(f"summary/TCN_r8{args.batch_size}_{args.learning_rate}_{args.num_epochs}_{args.downsample_factor}_{args.warm_up_epochs}.txt", "w") as f:
        f.write(summary_str)
