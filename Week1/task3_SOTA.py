...
for model in ["MOG", "MOG2", "KNN", "LSBP", "CNT"]:
    ...
    while cap.isOpened():
        ...
        # Save frames 20 to 80 for the GIF (convert from BGR to RGB)
        #if 220 <= frame_idx <= 280:
        if frame_idx in [220,225,230,235,240,245,250,255,260,265,270,275,280]:
    ...
    if frames_for_gif:
        pil_frames = [Image.fromarray(frame) for frame in frames_for_gif]
        gif_filename = f"{model}.gif"
        # Duration is in milliseconds per frame; using fps from the video
        duration = int(1000 / fps) if fps > 0 else 100
        pil_frames[0].save(gif_filename, save_all=True, append_images=pil_frames[1:], optimize=True, duration=duration*2, loop=0)
